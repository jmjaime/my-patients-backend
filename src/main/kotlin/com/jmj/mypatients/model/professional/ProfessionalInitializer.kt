...
class ProfessionalInitializer(private val professionals: Professionals,
    ...
    private fun accountFor(professional: Professional) {
        accounts.findByProfessional(professional.id)?.run { throw ObjectAlreadyExistsException("Already exists an account for professional ${professional.id}") }
        accounts.save(Account(professional.id))
    }
    ...
    private fun accountFor(professional: Professional, office: Office) {
        ...
        val officeAccount = OfficeAccount(idGenerator(), professional.id, office.id)
    ...
    private fun accountFor(professional: Professional, patientSource: PatientSource) {
        ...
        val patientSourceAccount = PatientSourceAccount(idGenerator(), professional.id, patientSource.id)
